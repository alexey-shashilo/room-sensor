...
/* SHT45 runtime (portable). Owns the non-blocking single-shot measurement state
   ...
   then waits the measurement interval before the next cycle. There is no
   "data-ready" poll; a read before the conversion deadline NACKs, which the
   runtime treats as NOT-READY (not an error). Only real communication/CRC
   failures are counted as errors. */

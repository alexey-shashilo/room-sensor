 ...
 *
 * This is a faithful port of the authoritative Sensirion algorithm
 * (gas-index-algorithm, master, sensirion_gas_index_algorithm_fixpoint,
 * v3.2.0), BSD 3-Clause. The fixed-point (fix16_t) arithmetic is derived from
 * libfixmath (MIT). See gas_index.h for the full attribution and license.
 *
 * The algorithm is integer-only (fix16_t = int32_t). The F16() macro folds
 * compile-time floating constants into fix16_t at build time; there is no
 * runtime floating point, no dynamic allocation, and no STM32 HAL dependency.
 *
 * IMPORTANT (sampling contract): the algorithm assumes a constant ~1 Hz
 * sampling interval (GasIndexAlgorithm_SAMPLING_INTERVAL). It must NOT be
 * called at an arbitrary App cadence.
 *
 * Copyright (c) 2022, Sensirion AG — BSD 3-Clause. Reproduced under this
 * repository's MIT license; see LICENSE (compatible permissive licenses).
 */
...
static fix16_t fix16_mul(fix16_t inArg0, fix16_t inArg1)
{
    ...
    return result;
#else
    /* Adding 0x8000 (= 0.5) and then using right shift achieves proper
       rounding to result. Handle carry from lower to upper part. */
...
{
    /* This uses the basic binary restoring division algorithm. It appears to be
       faster to do the whole division manually than trying to compose a 64-bit
       divide out of 32-bit divisions on platforms without hardware divide. */
    ...
    {
        /* Perform one step manually to avoid overflows later. We know that
           divider's bottom bit is 0 here. */
...
static fix16_t fix16_sqrt(fix16_t x)
{
    ...
    /* The main part is executed twice, in order to avoid using 64 bit values
       in computations. */
    for (n = 0; n < 2; n++)
    {
        ...
        if (n == 0)
        {
            ...
            if (num > 65535)
            {
                /* The remainder 'num' is too large to be shifted left by 16, so
                   we have to add 1 to result manually and adjust 'num'
                   accordingly. num = a - (result + 0.5)^2
...
static fix16_t GasIndexAlgorithm__mox_model__process(
    GasIndexAlgorithmParams *params, fix16_t sraw);
static void GasIndexAlgorithm__sigmoid_scaled__set_parameters(
    GasIndexAlgorithmParams *params, fix16_t X0, fix16_t K,
    fix16_t offset_default);
static fix16_t GasIndexAlgorithm__sigmoid_scaled__process(
    GasIndexAlgorithmParams *params, fix16_t sample);
static void GasIndexAlgorithm__adaptive_lowpass__set_parameters(
    GasIndexAlgorithmParams *params);
static fix16_t GasIndexAlgorithm__adaptive_lowpass__process(
    GasIndexAlgorithmParams *params, fix16_t sample);
